    ...
    class CB_WP_Widget_Popular_Posts extends WP_Widget
    {
                ...
                while ($cb_qry->have_posts()) {
                    ...
                    <article <?php post_class('cb-article ' . esc_attr($cb_type) . ' ' . esc_attr($cb_style) . ' clearfix'); ?>>
                        ...
                        <?php if ($cb_type == 'cb-article-big') {
                            echo '<a href="' . get_the_permalink($cb_post_id) . '" class="cb-link"></a>';
                        } ?>
                    </article>
            ...
            if (($cb_filter_by == 'cb-visits') && (!empty($cb_qry))) {
                ...
                foreach ($cb_qry as $cb_post) {
                    ...
                    <article class="cb-article <?php echo esc_attr($cb_type) . ' ' . esc_attr($cb_style); ?> clearfix">
                        ...
                        <div class="cb-meta cb-article-meta">
                            <h4 class="cb-post-title"><a
                                        href="<?php echo esc_url($cb_post['post_permalink']); ?>"><?php echo esc_html($cb_post['post_title']); ?></a>
                            </h4>
        ...
        function form($instance)
        {
            ...
            <p><label for="<?php echo esc_attr($this->get_field_id('cb_title')); ?>">Title:</label>
                <input class="widefat" id="<?php echo esc_attr($this->get_field_id('cb_title')); ?>"
                       name="<?php echo esc_attr($this->get_field_name('cb_title')); ?>" type="text"
            ...
            <p><label for="<?php echo esc_attr($this->get_field_id('cb_number')); ?>">Number of posts to show:</label>
                <input id="<?php echo esc_attr($this->get_field_id('cb_number')); ?>"
                       name="<?php echo esc_attr($this->get_field_name('cb_number')); ?>" type="text"
                       value="<?php echo esc_attr($cb_number); ?>" size="3"/></p>
            ...
            <p><label for="<?php echo esc_attr($this->get_field_id('category')); ?>">Category:</label>
                <select id="<?php echo esc_attr($this->get_field_id('category')); ?>"
                    ...
                    <option value="cb-all" <?php if ($cb_category == 'all') echo 'selected="selected"'; ?>>All
                        Categories
                    </option>
                    <?php
                    foreach ($cb_categories as $cb_cat) {

                        if ($cb_category == $cb_cat->slug) {
            ...
            <p><label for="<?php echo esc_attr($this->get_field_id('cb_type')); ?>"><?php echo "Style:"; ?></label>
                ...
                <select id="<?php echo esc_attr($this->get_field_id('cb_type')); ?>"
                        name="<?php echo esc_attr($this->get_field_name('cb_type')); ?>">
                    <option value="cb-article-small" <?php if ($cb_type == 'cb-article-small') echo 'selected="selected"'; ?>>
                        Small
                    </option>
                    <option value="cb-article-big" <?php if ($cb_type == 'cb-article-big') echo 'selected="selected"'; ?>>
                        Big
                    </option>
            ...
            <p>
                <label for="<?php echo esc_attr($this->get_field_id('cb_filter_by')); ?>"><?php echo "Filter:"; ?></label>

                <select id="<?php echo esc_attr($this->get_field_id('cb_filter_by')); ?>"
                        name="<?php echo esc_attr($this->get_field_name('cb_filter_by')); ?>">
                    <option value="cb-comments" <?php if ($cb_filter_by == 'cb-comments') echo 'selected="selected"'; ?>>
                        Number of Comments
                    </option>
                    <option value="cb-visits" <?php if ($cb_filter_by == 'cb-visits') echo 'selected="selected"'; ?>>
                        Number of Views
                    </option>
                </select></p>
            ...
            <p>
                ...
                <select id="<?php echo esc_attr($this->get_field_id('cb_filter_date')); ?>"
                        name="<?php echo esc_attr($this->get_field_name('cb_filter_date')); ?>">
                    <option value="alltime" <?php if ($cb_filter_date == 'alltime') {
                        echo 'selected="selected"';
                    } ?>>All-time
                    </option>
                    <option value="month" <?php if ($cb_filter_date == 'month') {
                        echo 'selected="selected"';
                    } ?>>Last Month
                    </option>
                    <option value="week" <?php if ($cb_filter_date == 'week') {
                        echo 'selected="selected"';
                    } ?>>Past 7 Days
                    </option>
                </select>
