...
if ($cb_qry->have_posts()) {
    ...
    while ($cb_qry->have_posts()) : $cb_qry->the_post();
        ...
        $i++;
        if ($i == 1) {
            echo '<div class="' . $cb_wrap . ' cb-module-half clearfix">' . $cb_title_header;
            ?>
            <article <?php post_class('cb-article cb-article-row cb-img-above-meta cb-no-1 cb-big cb-separated clearfix'); ?>>
