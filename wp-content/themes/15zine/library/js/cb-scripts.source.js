...
(function ($) {
    ...
    function cbScrolls() {
        ...
        if ((cbParallaxImg.length !== 0) && (cbMobileTablet === false)) {
            ...
            if (cbWindowScrollTop < cbWindowHeight) {
                ...
                cbParallaxImg.css({
                    '-webkit-transform': 'translate3d(0, ' + cbCoords + ', 0)',
                    'transform': 'translate3d(0, ' + cbCoords + ', 0)'
                });
            } else {
        ...
        if (ref.hasClass(cbScripts.cbFsClass)) {
            ...
            if (cbElement.hasClass('wp-caption')) {
                ...
                if (cbElementImg.hasClass('size-full')) {
                    ...
                    if (cbBodyRTL === true) {
                        cbElement.css({
                            'margin-right': ((cbPostEntryContent.width() / 2) - (cbWindowWidth / 2)),
                            'max-width': 'none'
                        }).addClass('cb-fs-embed');
                    } else {
                        cbElement.css({
                            'margin-left': ((cbPostEntryContent.width() / 2) - (cbWindowWidth / 2)),
                            'max-width': 'none'
                        }).addClass('cb-fs-embed');
                    }
    ...
    function cbAL() {
        ...
        $.ajax({
            ...
            data: {
                action: 'cb_alp_l',
                cbALlNonce: cbScripts.cbALlNonce,
                cbPostId: cbScripts.cbPostID,
                cbCounter: cbCounter
            },
            url: cbScripts.cbUrl,
            beforeSend: function () {
            ...
            success: function (data) {
                ...
                if ($('#cb-al-post-' + cbCounter + ' #cb-gallery-post').length) {
                    $('#cb-al-post-' + cbCounter + ' #cb-gallery-post').slick({
                        variableWidth: true,
                        centerMode: false,
                        rtl: cbBodyRTL,
                ...
                $('#cb-al-post-' + cbCounter + ' .cb-slider-1').flexslider({
                    maxItems: 1,
                    minItems: 1,
                    keyboard: true,
                ...
                if ($('#cb-al-post-' + cbCounter + ' .cb-slideshow-wrap').length) {
                    $('#cb-al-post-' + cbCounter + ' .cb-slideshow-wrap').slick({
                        infinite: true,
                        rtl: cbBodyRTL,
                        speed: 500,
                        fade: true,
    ...
    cbModFs.each(function () {
        var cbThis = $(this);
        cbThis.css({
            'margin-left': ((cbContent.width() / 2) - (cbWindowWidth / 2)),
            'max-width': 'none',
            'width': cbWindowWidth
        });
    });
    ...
    $(window).load(function () {
        ...
        if (cbTaggerLength === 4) {
            cbTabber.addClass("cb-tab-4");
        }
        if (cbTaggerLength === 3) {
            cbTabber.addClass("cb-tab-3");
        }
        if (cbTaggerLength === 2) {
            cbTabber.addClass("cb-tab-2");
        }
        if (cbTaggerLength === 1) {
            cbTabber.addClass("cb-tab-1");
        }
    ...
    function cbUserVote(element) {
        ...
        element.each(function () {
            ...
            if (cbVoteOverlay.length) {
                ...
                if (cbExistingOverlay !== '125px') {
                    cbExistingOverlaySpan.addClass('cb-zero-stars-trigger');
                }
            }
            ...
            cbNotVoted.on('mousemove click mouseleave mouseenter', function (e) {
                ...
                if (e.type == 'click') {
                    ...
                    if (cbThisUV.hasClass('points')) {
                        cbFinalX = cbFinalX * 10;
                    }
                    if (cbThisUV.hasClass('stars')) {
                        cbFinalX = cbFinalX * 20;
                    }
                    ...
                    $.ajax({
                        ...
                        success: function (data) {
                            ...
                            cbVoteCriteria.fadeOut(550, function () {
                                $(this).fadeIn(550).html(cbExistingVoteLine).find('.cb-votes-count').html(cbVotesText);
                            });
