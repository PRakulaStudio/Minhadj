...
if (!function_exists('cb_custom_code')) {
    function cb_custom_code()
    {
        ...
        if (ot_get_option('cb_custom_head', NULL) != NULL) {
            echo ot_get_option('cb_custom_head', NULL);
        }
        if ($cb_mm_columns_color != NULL) {
            $cb_custom_css .= '#cb-nav-bar .cb-mega-menu-columns .cb-sub-menu > li > a { color: ' . $cb_mm_columns_color . '; }';
        }
        if ($cb_bbp_sticky_background_color != NULL) {
            $cb_custom_css .= '.bbp-topics-front ul.super-sticky, .bbp-topics ul.super-sticky, .bbp-topics ul.sticky, .bbp-forum-content ul.sticky {background-color: ' . $cb_bbp_sticky_background_color . '!important;}';
        }
        ...
        if ($cb_custom_body_color_css != NULL) {
            $cb_custom_css .= 'body { color:' . $cb_custom_body_color_css . '; }';
        }
        if ($cb_footer_color != NULL) {
            $cb_custom_css .= '#cb-footer { color:' . $cb_footer_color . '; }';
        }
        if ($cb_custom_header_bg != NULL) {
            $cb_custom_css .= '.cb-header { ' . $cb_custom_header_bg_output . ' }';
        }
        if (ot_get_option('cb_mob_menu_bg', NULL) != NULL) {
            $cb_custom_css .= '.cb-mob-menu .cb-mob-bg { background-image: url(' . esc_url(ot_get_option('cb_mob_menu_bg', NULL)) . ') }';
        }
        if ($cb_body_font_size_mobile != NULL) {
            if ($cb_body_font_size_mobile[1] == NULL) {
                $cb_body_font_size_mobile[1] = 'px';
            }
            $cb_custom_css .= 'body { font-size: ' . $cb_body_font_size_mobile[0] . $cb_body_font_size_mobile[1] . '; }';
        }

        if ($cb_body_font_size_desktop != NULL) {
            if ($cb_body_font_size_desktop[1] == NULL) {
                $cb_body_font_size_desktop[1] = 'px';
            }
        ...
        if ($cb_light_menu_color != '#161616') {
            $cb_custom_css .= '.cb-menu-light #cb-nav-bar .cb-main-nav, .cb-stuck.cb-menu-light #cb-nav-bar .cb-nav-bar-wrap { border-bottom-color: ' . $cb_light_menu_color . '; }';
        }
        if ($cb_light_menu == 'off') {
            $cb_custom_css .= '.cb-menu-light #cb-nav-bar .cb-main-nav, .cb-stuck.cb-menu-light #cb-nav-bar .cb-nav-bar-wrap, .cb-menu-light.cb-fis-big-border #cb-nav-bar .cb-nav-bar-wrap { border-bottom-color: transparent; }';
        }
        if ($cb_base_color != NULL) {
            $cb_custom_css .= '.cb-mm-on.cb-menu-light #cb-nav-bar .cb-main-nav > li:hover, .cb-mm-on.cb-menu-dark #cb-nav-bar .cb-main-nav > li:hover { background: ' . $cb_base_color . '; }';
        }
        if ($cb_logo_nav_left != 0) {
            $cb_custom_css .= '#cb-nav-logo { margin-right: ' . $cb_logo_nav_left . 'px!important; }';
        }
        if ($cb_custom_css != NULL) {
            echo '<style type="text/css">' . $cb_custom_css . '</style><!-- end custom css -->';
        }
...
if (!function_exists('cb_custom_footer_code')) {
    function cb_custom_footer_code()
    {
        ...
        if (ot_get_option('cb_custom_footer', NULL) != NULL) {
            echo ot_get_option('cb_custom_footer', NULL);
        }
        if (ot_get_option('cb_disqus_shortname', NULL) != NULL) {
            echo "<script type='text/javascript'>var disqus_shortname = '" . ot_get_option('cb_disqus_shortname', NULL) . "'; // required: replace example with your forum shortname
                        (function () {
...
if (!function_exists('cb_get_post_meta')) {
    function cb_get_post_meta($cb_post_id, $cb_override = NULL)
    {
        ...
        $cb_comments = $cb_views = $cb_cat_output = $cb_readtime_output = NULL;
...
if (!function_exists('cb_byline')) {
    function cb_byline($cb_post_id, $cb_cat = NULL, $cb_is_post = NULL, $cb_read_time = NULL, $cb_views_on = NULL)
    {
        echo cb_get_byline($cb_post_id, $cb_cat = NULL, $cb_is_post = NULL, $cb_read_time = NULL, $cb_views_on = NULL);
    }
}
...
if (!function_exists('cb_get_byline')) {
    function cb_get_byline($cb_post_id, $cb_cat = NULL, $cb_is_post = NULL, $cb_read_time = NULL, $cb_views_on = NULL, $cb_override = NULL)
    {
        ...
        if (($cb_meta_onoff == 'on') || ($cb_override == 'on') || (($cb_meta_onoff == 'on_posts') && (is_single()))) {
        ...
        function start_el(&$output, $item, $depth = 0, $args = array(), $id = 0)
        {
            ...
            <dl class="menu-item-bar">
                <dt class="menu-item-handle">
                    <span class="item-title"><span class="menu-item-title"><?php echo esc_html($title); ?></span> <span
                                class="is-submenu" <?php echo esc_attr($submenu_text); ?>>sub item</span></span>
                    <span class="item-controls">
                            ...
                            <span class="item-order hide-if-js">
                                ...
                                ?>" class="item-move-up"><abbr
                                            title="<?php esc_attr_e('Move up'); ?>">&#8593;</abbr></a>
                                |
                                <a href="<?php
                                echo wp_nonce_url(
                                    add_query_arg(
            ...
            <div class="menu-item-settings" id="menu-item-settings-<?php echo esc_attr($item_id); ?>">
                ...
                <p class="field-cbmegamenu description description-thin">
                    <label for="edit-menu-item-cbmegamenu-<?php echo esc_attr($item_id); ?>">15Zine Megamenu
                        Type</label>
                    <select id="edit-menu-item-cbmegamenu-<?php echo esc_attr($item_id); ?>"
                            name="menu-item-cbmegamenu[<?php echo esc_attr($item_id); ?>]">
                        <option value="1" <?php if (($cb_item_megamenu == '1') || ($cb_item_megamenu == NULL)) echo 'selected="selected"'; ?>>
                            Normal Menu
                        </option>
                        <?php if ($item->object == 'category') { ?>
                            <option value="3" <?php if ($cb_item_megamenu == '3') echo 'selected="selected"'; ?>>15Zine
                                Category/Posts Megamenu
                            </option>
                        <?php } ?>
                        <option value="2" <?php if ($cb_item_megamenu == '2') echo 'selected="selected"'; ?>>15Zine Text
                            Columns Megamenu
                        </option>
                    </select>
                </p>
                ...
                <div class="menu-item-actions description-wide submitbox">
                    ...
                    <a class="item-delete submitdelete deletion" id="delete-<?php echo esc_attr($item_id); ?>"
                       ...
                       ); ?>"><?php _e('Remove', 'cubell'); ?></a> <span class="meta-sep hide-if-no-js"> | </span> <a
...
if (!class_exists('cb_mega_walker')) {
    class cb_mega_walker extends Walker_Nav_Menu
    {
        ...
        function start_lvl(&$output, $depth = 0, $args = array())
        {
            ...
            if ($depth == 2) {
                $output .= '<ul class="cb-grandchild-menu cb-great-grandchild-menu cb-sub-bg">';
            }
            if ($depth == 1) {
                $output .= '<ul class="cb-grandchild-menu cb-sub-bg">';
            }
            if ($depth == 0) {
                $output .= '<ul class="cb-sub-menu cb-sub-bg">';
            }
        }
        ...
        <article id="comment-<?php comment_ID(); ?>" class="clearfix">
            ...
            <div class="cb-comment-body clearfix">
                ...
                <header class="comment-author vcard">
                    ...
                    <time datetime="<?php comment_date(); ?>"><a
                                href="<?php echo htmlspecialchars(get_comment_link($comment->comment_ID)) ?>"><?php comment_date(); ?> </a>
                    </time>
...
if (!function_exists('cb_contact_data')) {
    function cb_contact_data($contactmethods)
    {
        ...
        if (is_admin() == true) {
            ...
            $contactmethods['facebook'] = '15Zine: Facebook (Entire URL)';
...
if (!function_exists('cb_get_fis_tl')) {
    function cb_get_fis_tl($cb_post_id)
    {
        ...
        if (($cb_featured_image_style_override_onoff == 'on') && ($cb_featured_image_style_override_style == 'parallax')) {
            $cb_title_loc = 'cb-fis-tl-overlay';
        }
 ...
 * READING TIME
 *********************/
if (!function_exists('cb_reading_time')) {
    function cb_reading_time()
    {

        if ((ot_get_option('cb_byline_readtime', false) == false)) {
            return;
        }

        $cb_word_count = floor(str_word_count(get_the_content()) / 190);

        return sprintf(_n('%d min read', '%d min read', $cb_word_count, 'cubell'), $cb_word_count);
...
if (!function_exists('cb_get_featured_image_style')) {
    function cb_get_featured_image_style($cb_featured_image_style, $post, $cb_page = NULL)
    {
        ...
        if (ot_get_option('cb_caption_onoff', 'on') == 'on') {
        ...
        if (($cb_post_format == 'gallery') && ($cb_gallery_post_images != NULL)) {
            ...
            if ($cb_featured_image_url != NULL) {
                ...
                if ($cb_get_fis_tl == 'cb-fis-tl-me-above') {
                    ...
                    $cb_output .= '<div class="cb-fis-bg" style="background-image: url( ' . esc_url($cb_featured_image_url[0]) . ');"></div>';
                    $cb_output .= '</div>';
                    $cb_output .= '<div class="cb-fis-title-bg ' . cb_get_post_sidebar_position($cb_post_id) . ' clearfix wrap">' . $cb_title . $cb_title_closer . '</div>';
...
if (!function_exists('cb_review_boxes')) {
    function cb_review_boxes($post)
    {
        ...
        if ($cb_review_checkbox == 'on') {
            ...
            if (isset ($cb_custom_fields['cb_ct1'][0])) {
                $cb_rating_1_title = $cb_custom_fields['cb_ct1'][0];
            }
            if (isset ($cb_custom_fields['cb_cs1'][0])) {
                $cb_rating_1_score = $cb_custom_fields['cb_cs1'][0];
            }
            if (isset ($cb_custom_fields['cb_ct2'][0])) {
                $cb_rating_2_title = $cb_custom_fields['cb_ct2'][0];
            }
            if (isset ($cb_custom_fields['cb_cs2'][0])) {
                $cb_rating_2_score = $cb_custom_fields['cb_cs2'][0];
            }
            if (isset ($cb_custom_fields['cb_ct3'][0])) {
                $cb_rating_3_title = $cb_custom_fields['cb_ct3'][0];
            }
            if (isset ($cb_custom_fields['cb_cs3'][0])) {
                $cb_rating_3_score = $cb_custom_fields['cb_cs3'][0];
            }
            if (isset ($cb_custom_fields['cb_ct4'][0])) {
                $cb_rating_4_title = $cb_custom_fields['cb_ct4'][0];
            }
            if (isset ($cb_custom_fields['cb_cs4'][0])) {
                $cb_rating_4_score = $cb_custom_fields['cb_cs4'][0];
            }
            if (isset ($cb_custom_fields['cb_ct5'][0])) {
                $cb_rating_5_title = $cb_custom_fields['cb_ct5'][0];
            }
            if (isset ($cb_custom_fields['cb_cs5'][0])) {
                $cb_rating_5_score = $cb_custom_fields['cb_cs5'][0];
            }
            if (isset ($cb_custom_fields['cb_ct6'][0])) {
                $cb_rating_6_title = $cb_custom_fields['cb_ct6'][0];
            }
            if (isset ($cb_custom_fields['cb_cs6'][0])) {
                $cb_rating_6_score = $cb_custom_fields['cb_cs6'][0];
            }
            ...
            if ($cb_score_display_type == 'percentage') {
                ...
                for ($i = 1; $i < 7; $i++) {
                    if (isset(${"cb_rating_" . $i . "_score"})) {
                        $cb_ratings[] = ${"cb_rating_" . $i . "_score"} / 10;
                    }
                }
            } else {
            ...
            if (($cb_review_type == 'cb-both') || ($cb_review_type == 'cb-readers') || ($cb_review_type == 'on')) {
                ...
                if ($cb_number_votes == NULL) {
                    $cb_number_votes = 0;
                }
                if ($cb_user_score == NULL) {
                    $cb_user_score = 0;
                }
                ...
                if ($cb_review_type == 'cb-readers') {
                    ...
                    if ($cb_score_display_type == "points") {
                        $cb_average_score = '<div class="cb-criteria-score cb-average-score"><span itemprop="ratingValue">' . number_format(floatval($cb_user_score / 10), 1) . '</span>
      <meta itemprop="bestRating" content="' . $cb_best_rating . '"><meta itemprop="worstRating" content="0"></div>';
                    }
                    if ($cb_score_display_type == "percentage") {
                        $cb_average_score = '<div class="cb-criteria-score cb-average-score"><span itemprop="ratingValue">' . $cb_user_score . '%' . '</span>
      <meta itemprop="bestRating" content="' . $cb_best_rating . '"><meta itemprop="worstRating" content="0"></div>';
                    }
                    ...
                    if ($cb_score_display_type == "points") {
                        $cb_average_score = '<div class="cb-criteria-score cb-average-score">' . number_format(floatval($cb_user_score / 10), 1) . '</div>';
                    }
                    if ($cb_score_display_type == "percentage") {
                        $cb_average_score = '<div class="cb-criteria-score cb-average-score">' . $cb_user_score . '%</div>';
                    }
    ...
    function cb_get_review_ext_box($cb_post_id = NULL, $cb_small_box = false)
    {
        ...
        if (($cb_review_checkbox == 'on') || ($cb_review_checkbox == '1')) {
            ...
            if (class_exists('Lets_Review_API')) {
                ...
                if ($lets_review_api->lets_review_get_onoff($cb_post_id) == false) {
                    return;
                }
...
if (!function_exists('cb_author_details')) {
    function cb_author_details($cb_author_id, $cb_class = NULL)
    {
        ...
        $cb_author_facebook = get_the_author_meta('facebook', $cb_author_id);
        ...
        if ($cb_author_position != NULL) {
            $cb_author_output .= '<div class="cb-author-position cb-font-header">' . $cb_author_position . '</div>';
        }

        if ($cb_author_desc != NULL) {
            $cb_author_output .= '<p class="cb-author-bio">' . $cb_author_desc . '</p>';
        }

        if (($cb_author_email != NULL) || ($cb_author_www != NULL) || ($cb_author_facebook != NULL) || ($cb_author_tw != NULL) || ($cb_author_instagram != NULL) || ($cb_author_googleplus != NULL)) {
            $cb_author_output .= '<div class="cb-author-page-contact">';
        }
        if ($cb_author_email != NULL) {
            $cb_author_output .= '<a href="mailto:' . sanitize_email($cb_author_email) . '" class="cb-contact-icon cb-tip-bot" data-cb-tip="' . __('Email', 'cubell') . '"><i class="fa fa-envelope-o"></i></a>';
        }
        if ($cb_author_www != NULL) {
            $cb_author_output .= ' <a href="' . esc_url($cb_author_www) . '" target="_blank" class="cb-contact-icon cb-tip-bot" data-cb-tip="' . __('Website', 'cubell') . '"><i class="fa fa-link"></i></a> ';
        }
        if ($cb_author_tw != NULL) {
            $cb_author_output .= ' <a href="//www.twitter.com/' . $cb_author_tw . '" target="_blank" class="cb-contact-icon cb-tip-bot" data-cb-tip="Twitter"><i class="fa fa-twitter"></i></a>';
        }
        if ($cb_author_googleplus != NULL) {
            $cb_author_output .= ' <a href="' . esc_url($cb_author_googleplus) . '" target="_top" class="cb-contact-icon cb-tip-bot" data-cb-tip="Google+"><i class="fa fa-google-plus"></i></a>';
        }
        if ($cb_author_facebook != NULL) {
            $cb_author_output .= ' <a href="' . esc_url($cb_author_facebook) . '" target="_top" class="cb-contact-icon cb-tip-bot" data-cb-tip="Facebook"><i class="fa fa-facebook"></i></a>';
        }
        if ($cb_author_instagram != NULL) {
            $cb_author_output .= ' <a href="' . esc_url($cb_author_instagram) . '" target="_top" class="cb-contact-icon cb-tip-bot" data-cb-tip="Instagram"><i class="fa fa-instagram"></i></a>';
        }
        if (($cb_author_email != NULL) || ($cb_author_www != NULL) || ($cb_author_facebook != NULL) || ($cb_author_instagram != NULL) || ($cb_author_googleplus != NULL) || ($cb_author_tw != NULL)) {
...
if (!function_exists('cb_about_author')) {
    function cb_about_author($post, $cb_author_id_sc = NULL)
    {
        ...
        $cb_author_output .= '<div class="cb-title cb-font-header vcard"><a href="' . get_author_posts_url($cb_author_id) . '"><span class="fn">' . $cb_author_name . '</span></a>';
        $cb_author_output .= '</div>';
        if ($cb_author_position != NULL) {
            $cb_author_output .= '<div class="cb-author-position cb-font-header">' . $cb_author_position . '</div>';
        }
        ...
        if (($cb_author_email != NULL) || ($cb_author_www != NULL) || ($cb_author_tw != NULL) || ($cb_author_instagram != NULL)) {
            $cb_author_output .= '<div class="cb-author-page-contact">';
        }
        if ($cb_author_email != NULL) {
            $cb_author_output .= '<a href="mailto:' . sanitize_email($cb_author_email) . '" class="cb-contact-icon cb-tip-bot" data-cb-tip="' . __('Email', 'cubell') . '"><i class="fa fa-envelope-o"></i></a>';
        ...
        if (($cb_author_email != NULL) || ($cb_author_www != NULL) || ($cb_author_instagram != NULL) || ($cb_author_googleplus != NULL) || ($cb_author_tw != NULL)) {
            $cb_author_output .= '</div>';
        }
        $cb_author_output .= '</div>';
        ...
        if (($cb_user_info->user_level > 8) && (is_admin() == true)) {
            ...
            <table class="form-table cb-about-options">
                <tr>
                    ...
                    <td>
                        <input type="checkbox" name="cb_show_author"
                               id="cb_show_author" <?php if ((get_the_author_meta("cb_show_author", $cb_user->ID) == 1) || (get_the_author_meta("cb_show_author", $cb_user->ID) == "true")) echo "checked"; ?> />
                    </td>
                </tr>
                <tr>
                    ...
                    <td>
                        <select name="cb_order" id="cb_order">
                            <option value="0" <?php if ($cb_saved == "0") {
                                echo 'selected="selected"';
                            } ?>>Alphabetical
                            </option>
                            <option value="1" <?php if ($cb_saved == "1") {
                                echo 'selected="selected"';
                            } ?>>1
                            </option>
                            <option value="2" <?php if ($cb_saved == "2") {
                                echo 'selected="selected"';
                            } ?>>2
                            </option>
                            <option value="3" <?php if ($cb_saved == "3") {
                                echo 'selected="selected"';
                            } ?>>3
                            </option>
                            <option value="4" <?php if ($cb_saved == "4") {
                                echo 'selected="selected"';
                            } ?>>4
                            </option>
                            <option value="5" <?php if ($cb_saved == "5") {
                                echo 'selected="selected"';
                            } ?>>5
                            </option>
                        </select>
...
if (!function_exists('cb_extra_profile_about_us_save')) {
    function cb_extra_profile_about_us_save($cb_user)
    {
        ...
        if (!current_user_can('edit_user', $cb_user)) {
            return false;
        }
        $cb_current_user = get_current_user_id();
        $cb_user_info = get_userdata($cb_current_user);
        ...
        if (($cb_user_info->user_level) > 8 && (is_admin() == true)) {
            $cb_show_author_data = (isset($_POST['cb_show_author']) && $_POST['cb_show_author']) ? true : false;
...
if (!function_exists('cb_backgrounds')) {
    function cb_backgrounds()
    {
        ...
        if (($cb_bg_color != NULL) || ($cb_bg_img != NULL)) {
            $cb_output .= '<!-- Body BG --><style>@media only screen and (min-width: 1200px) { body, .cb-first-alp {' . $cb_bg_img . $cb_bg_color . $cb_body_bg_repeat . '} .cb-body-light .cb-post-wrap { background: #fff; } .cb-body-dark .cb-post-wrap { background: #161616; } }</style>';
        }
...
if (!function_exists('cb_logo')) {
    function cb_logo()
    {
        ...
        $cb_logo_h1 = ot_get_option('cb_logo_h1', 'off');

        if ($cb_logo != NULL) {

            ?>
            <div id="logo">
                <?php if (($cb_logo_h1 == 'on') && (is_front_page())) {
                    echo '<h1>';
                } ?>
                <a href="<?php echo esc_url(home_url()); ?>">
                    <img src="<?php echo esc_url($cb_logo); ?>"
                         alt="<?php echo esc_attr($cb_logo_alt); ?>" <?php if ($cb_retina_logo != NULL) {
                        echo 'data-at2x="' . esc_url($cb_retina_logo) . '"';
                    } ?>>
                </a>
                <?php if (($cb_logo_h1 == 'on') && (is_front_page())) {
                    echo '</h1>';
    ...
    function cb_mob_logo()
    {
        ...
        if ($cb_logo != NULL) {
            ...
            <div id="mob-logo" class="cb-top-logo">
                <a href="<?php echo esc_url(home_url()); ?>">
                    <img src="<?php echo esc_url($cb_logo); ?>"
                         alt="<?php esc_html(get_bloginfo('name')); ?> logo" <?php if ($cb_retina_logo != NULL) {
                        echo 'data-at2x="' . esc_url($cb_retina_logo) . '"';
                    } ?>>
                </a>
            </div>
...
function cb_woo_end_wrap()
{
    ...
    if (ot_get_option('cb_woocommerce_sidebar', 'sidebar') != 'nosidebar-fw') {
        get_sidebar();
    }
    ...
    function cb_modal_logo()
    {
        ...
        if ($cb_logo != NULL) {
            ...
            <div class="cb-lwa-logo cb-ta-center">
                <img src="<?php echo esc_url($cb_logo); ?>"
                     alt="<?php esc_html(get_bloginfo('name')); ?> logo" <?php if ($cb_logo_retina != NULL) {
                    echo 'data-at2x="' . $cb_logo_retina . '"';
                } ?>>
            </div>
            <?php
        }
        ...
        <ul id="<?php echo esc_attr($field_id); ?>" class="cb-ot-dragger cb-sortable">
            <?php if (!empty($cb_dragger_ops)) { ?>
                <?php foreach ($cb_dragger_ops as $field_setting) { ?>
                    ...
                    <li id="cb-dragger-<?php echo intval($i); ?>"
                        class="ui-state-default cb-list-field <?php echo esc_attr($field_setting); ?>-field cb-clearfix">
                        Section <span class="cb-uppercase"><?php echo esc_attr($field_setting); ?></span>
                        <input type="hidden" value="<?php echo esc_attr($field_setting); ?>"
                               id="cb-hp-dragger-<?php echo intval($i); ?>"
                               name="cb_hp_dragger_option[<?php echo intval($i); ?>]" class="cb-input">
                    </li>
                    <?php $i++; ?>
 ...
 * ALP LOADER
 *********************/
if (!function_exists('cb_alp_ldr')) {
    function cb_alp_ldr()
    {
        if (ot_get_option('cb_postload_onoff', 'off') != 'off') {
            ?>
            <span id="cb-alp-loader" class="cb-alp-ld"><i class="fa fa-circle-o-notch fa-3x fa-fw"></i><span
                        class="cb-end"><?php echo __('No more articles', 'cubell'); ?></span></span>
            <?php
        }
    }
}
    ...
    function cb_alp_l()
    {
        ...
        if ($cb_qry->have_posts()) {
            ...
            while ($cb_qry->have_posts()) {
                ...
                ?>
                <div id="cb-al-post-<?php echo absint($_GET['cbCounter']); ?>"
                     class="cb-entire-post cb-al-post cb-slide-up-opa cb-alp-aj clearfix<?php echo esc_attr($cb_fis_size); ?>"
                     data-cb-purl="<?php echo esc_url(get_permalink($cb_post_id)); ?>"
                     data-cb-pid="<?php echo intval($cb_post_id); ?>">

                    <?php if (($cb_featured_image_style != 'off') && ($cb_featured_image_style_cache != 'off') && ($cb_featured_image_style != 'standard') && ($cb_featured_image_style_cache != 'standard')) {
                        do_shortcode(cb_featured_image_style($cb_featured_image_style, $post));
                    } ?>

                    <div class="cb-post-wrap cb-wrap-pad wrap clearfix<?php echo esc_attr(cb_get_post_sidebar_position($cb_post_id));
                    echo esc_attr($cb_get_singular_fs); ?>">
                        ...
                        <div class="cb-main">
                            ...
                            <article id="post-<?php the_ID(); ?>" <?php post_class('clearfix'); ?>>
                                ...
                                <?php if (($cb_featured_image_style == 'off') || ($cb_featured_image_style == 'standard') || ($cb_featured_image_style_cache == 'standard') || ($cb_featured_image_style_cache == 'off')) {
                        ...
                        <?php if (($cb_sidebar_position != 'nosidebar') && ($cb_sidebar_position != 'nosidebar-fw')) {
                            get_sidebar();
                        } ?>
                    </div>
    ...
    function cb_get_thumbnail_cover($width, $height, $cb_post_id = NULL)
    {
        ...
        if ((has_post_thumbnail($cb_post_id)) && (get_the_post_thumbnail($cb_post_id) != NULL)) {
            ...
            if ($cb_thumbnail == NULL) {
                $cb_thumbnail = wp_get_attachment_image_src(get_post_thumbnail_id($cb_post_id), $cb_dimensions);
            }
...
if (!function_exists('cb_schema_meta')) {
    function cb_schema_meta($post)
    {
        ...
        <span class="cb-hide" itemprop="author" itemscope itemtype="https://schema.org/Person"><meta itemprop="name"
                                                                                                     content="<?php echo get_the_author_meta('display_name', $cb_author_id); ?>"></span>
