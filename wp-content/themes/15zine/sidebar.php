...
if (is_category()) {
    ...
    $cb_sidebar_custom = get_post_meta($cb_page_id, 'cb_page_custom_sidebar', true);

    if ($cb_sidebar_custom == 'on') {
        $cb_sidebar_select = get_post_meta($cb_page_id, 'cb_page_custom_sidebar_type', true);
        if ($cb_sidebar_select == 'cb_existing') {
            $cb_sidebar_id_check = get_post_meta($cb_page_id, 'cb_sidebar_select', true);
...
<?php if (ot_get_option('cb_sticky_sb', 'on') == 'on') {
    echo '</div>';
} ?>